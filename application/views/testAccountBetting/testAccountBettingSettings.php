...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#save_edit_btn").on("click", function() {
			...
			if(risefall_minimum == ""||contract_minimum == ""){
				...
				var res = ajaxShortLink("test-account/saveBettingSettings",{
	...
	$('#tableContainer').on('click', 'tbody tr', function () {
		selectedData = $('#tableContainer').DataTable().row($(this)).data();
	  	...
	  	bootbox.alert({
	  	    message: ajaxLoadPage('quickLoadPage',{'pagename':'testAccountBetting/editTimingIncome'}),
	  	    size: 'large',
	  	    centerVertical: true,
	  	    closeButton: false
	  	});
	...
	$('#add_option_btn').on('click', function () {
  	bootbox.alert({
  	    message: ajaxLoadPage('quickLoadPage',{'pagename':'testAccountBetting/addTimingIncome'}),
  	    size: 'large',
  	    centerVertical: true,
  	    closeButton: false
  	});
