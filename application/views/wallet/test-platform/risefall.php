...
<script type="text/javascript">
    ...
    //continous
        ...
        const pendingPositionChecker = setInterval(function() {
            ...
            var changes = getChanges(positionsOpened, oldPositionsOpened);

            console.log(positionsOpened,oldPositionsOpened,changes);
            ...
            if(changes.length>=1){
                ...
                for (var i = 0; i < changes.length; i++) {
                    var riseFallDetails = ajaxShortLink('userWallet/risefall/getRiseFallDetailsByID',{
                        'id':changes[i].id
                    })

                    console.log(riseFallDetails);

                    if(riseFallDetails.status == "WIN"){
                        var newIncome = ((riseFallDetails.income/100)*riseFallDetails.amount).toFixed(2);
                        ...
                        $.toast({
                            heading: '<h6>LOST!</h6>',
                            text: 'You have lost '+riseFallDetails.amount+' USDT',
                            showHideTransition: 'slide',
                            icon: 'error',
                    ...
                    if(positionsOpened.length==0){
                        ...
                        if (positionsOpened[x].income == 30) {
                            timing = '30/30'
                        }else if(positionsOpened[x].income == 50){
                            timing = '60/50'
                        }else if(positionsOpened[x].income == 70){
                            timing = '120/70'
                        }else if(positionsOpened[x].income == 90){
                            timing = '180/90'
                        }
                    ...
                    if (riseFallDetails.income == 30) {
                        timing = '30/30'
                    }else if(riseFallDetails.income == 50){
                        timing = '60/50'
                    }else if(riseFallDetails.income == 70){
                        timing = '120/70'
                    }else if(riseFallDetails.income == 90){
                        timing = '180/90'
                    }
                    
                    $("#positions_closed_container").prepend(
                        '<tr class="'+statusClass+'">'+
                            '<td class="">'+riseFallDetails.buyType+' @ '+timing+'</td>'+
                            '<td class="">'+riseFallDetails.timeStamp+'</td>'+
                            '<td class="">'+riseFallDetails.amount+'</td>'+
                            '<td class="">'+riseFallDetails.currentPrice+'</td>'+
                            '<td class="">'+parseFloat(riseFallDetails.resolvedPrice).toFixed(2)+'</td>'+
                            '<td class="">'+riseFallDetails.status+' </td>'+
                        '</tr>'
                    ); 
                }
